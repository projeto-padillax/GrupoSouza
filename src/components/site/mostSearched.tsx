    ...
    <section className="py-12 justify-items-center">
      <div className="px-8 sm:px-10 md:px-12 w-full max-w-7xl">
        ...
        <div className="text-center md:text-start grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {links.map((item) => (
            <Link href={item.url} key={item.id} className="hover:text-[#4F7DC3] cursor-pointer hover:underline text-lg w-fit justify-self-center md:justify-self-start">{item.titulo}</Link>
          ))}
        </div>
      </div>
    </section>
