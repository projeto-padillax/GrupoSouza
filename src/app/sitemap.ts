...
const baseParams: Record<string, string> = {
  ...
  let url;
// Se houver categoria, adiciona ao query e à URL
if (categoriaSlug) {
  baseParams.tipos = categoriaSlug;
  url = `${process.env.NEXT_PUBLIC_BASE_URL}/busca/${action}/${categoriaSlug}/${cidadeSlug}?${buildQuery(baseParams)}`;
} else {
  url = `${process.env.NEXT_PUBLIC_BASE_URL}/busca/${action}/imoveis/${cidadeSlug}?${buildQuery(baseParams)}`;
}
